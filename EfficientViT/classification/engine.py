...
def evaluate(data_loader, model, device):
    ...
    all_preds = torch.cat(all_preds).numpy()    # shape: (N, num_pathologies)
    all_targets = torch.cat(all_targets).numpy()  # shape: (N, num_pathologies)
    ...
    auc_per_pathology = []
    num_pathologies = all_targets.shape[1]
    for i in range(num_pathologies):
        try:
            auc_i = roc_auc_score(all_targets[:, i], all_preds[:, i])
        except ValueError:
            auc_i = 0.0
        auc_per_pathology.append(auc_i)

    metric_logger.synchronize_between_processes()
    print(
        '* Accuracy: {acc:.3f} loss: {loss:.3f} '
        'f1_micro: {f1:.3f} auc_micro: {auc:.3f}'
        .format(
            acc=metric_logger.accuracy.global_avg,
    ...
    for name, auc_val in zip(pathology_names, auc_per_pathology):
    ...
    metrics['auc_per_pathology'] = auc_per_pathology
